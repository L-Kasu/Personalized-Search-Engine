...
def init_langfile(langfile=languageconfigfile, path=default_path):
    langconfig = {"english": {"ID_lang": "english",
                              ...
                              "txt_docs_to_return": "Number of documents",
                              "txt_on": "on",
                              "txt_off": "off",
                              "txt_confirm": "Confirm",
                              "txt_cancel": "Cancel",
                              ...
                              "txt_tooltip_logistic regression": "logistic regression is a machine-learning model. It first learns from a practice-set considering different factors (for example the tfidf-value (see tooltip regarding tfidf)), to predict if a query matches a document. After that it will try to predict the probability of a query matching a document. If you now enter a query, the documents will be ranked by probability to match the entered query.",
                  ...
                  "german": {"ID_lang": "german",
                             ...
                             "txt_docs_to_return": "Anzahl Dokumente",
                             "txt_on": "an",
                             "txt_off": "aus",
                             ...
                             "txt_tooltip_logistic regression": "Logistic Regression ist ein Machine-learning Modell. Dieses wird angewandt, indem dieses zuerst mit einem Trainingsset bestehend aus verschiedenen Faktoren (wie z.B. dem Tfidf-Wert (siehe tooltip zu tfidf)) lernt, wann eine Suchanfrage zu einem Dokument passt. Nach dem Training versucht es, eine Wahrscheinlichkeit dafür anzugeben. Es werden nun die Dokumente dannach geordnet, welche nach dem Modell die höchste Wahrscheinlichkeit besitzen, zu der Suchanfrage zu passen.",                             "ERR_noDirectorySelected": "FEHLER: Kein Verzeichnis ausgewählt",
                  ...
                  "spanish": {"ID_lang": "spanish",
                              ...
                              "txt_resultitems": "Resultados",
                              "txt_preview": "Vista previa",
                              "txt_page": "Pagina",
                              "txt_okay": "Okey",
                              "txt_exitpreview": "Cerrar",
                              "txt_language": "Idioma",
                              "txt_colortheme": "Tema",
                              "txt_font": "Tipo de letra",
                              "txt_selectStemmer": "Seleccione el Stemmer",
                              "txt_toggleStopword": "Eliminación de Stop Words",
                              "txt_toggleClustering": "Clustering",
                              "txt_selectsearchmode": "Seleccione el método de búsqueda",
                              "txt_selectsnowballlang": "seleccionar el idioma del Snowball Stemmer",
                              "txt_selectstopwordlang": "Idioma de Stop Words",
                              "txt_docs_to_return": "Número de documentos",
                              "txt_on": "on",
                              "txt_off": "off",
                              "txt_confirm": "Confirmar",
                              "txt_cancel": "Cancelar",
                              "txt_tooltip_porter": "Este es el algoritmo Porter de separación de sufijos. Sigue el algoritmo presentado en Porter, M. 'An algorithm for suffix stripping'. Program 14.3 (1980): 130-137.",
                              "txt_tooltip_lancaster": "Un stemmer de palabras basado en el algoritmo de stemming de Lancaster (Paice/Husk). Sigue el algoritmo presentado en Paice, Chris D. 'Another Stemmer'. ACM SIGIR Forum 24.3 (1990): 56-61",
                              "txt_tooltip_snowball": "Esta es una implementación de los stemmers 'Snowball' creados por Martin Porter. Seleccione esta opción si desea separar documentos en alemán, árabe, danés, español, finlandés, francés, húngaro, inglés, italiano, noruego, portugués, rumano, ruso o sueco. El algoritmo inglés se presentó en Porter, M. 'An algorithm for suffix stripping'. Program 14.3 (1980): 130-137.",
                              "txt_tooltip_stemmer": "Un Stemmer es un algoritmo que reduce las palabras flexionadas o derivadas a su raíz (la parte de la palabra responsable de su significado léxico). Por ejemplo, la raíz de las palabras 'gato', 'parecido a un gato' y 'gato' sería 'gato'.",
                              "txt_tooltip_stopwords": "Stop words son palabras que se filtran antes de procesar un conjunto de datos (porque no contienen información relevante para la búsqueda) para acelerar el paso de procesamiento. Estas palabras pueden ser 'el', 'la', 'de', 'con', ...",
                              "txt_tooltip_clustering": "El Clustering es el proceso de agrupar los objetos de un determinado conjunto de datos por un determinado rasgo (algún tipo de similitud, en este caso las palabras se agrupan por la similitud del contexto en el que aparecen). En este caso se hace para acelerar la búsqueda",
                              "txt_tooltip_tfidf": "Term Frequency–Inverse Document Frequency (tf-idf) es una estadística numérica que pretende reflejar la importancia de una palabra en un solo documento de una colección de documentos.  Aquí se utiliza como factor de ponderación para la búsqueda. El valor tf-idf aumenta proporcionalmente al número de veces que una palabra aparece en el documento y se compensa con el número de documentos de la colección que la contienen, lo que ayuda a ajustar el hecho de que algunas palabras aparecen con más frecuencia en general.",
                              "txt_tooltip_GloVe": "GloVe es un algoritmo de aprendizaje no supervisado para obtener representaciones vectoriales de las palabras. El entrenamiento se realiza a partir de las estadísticas globales de co-ocurrencia de palabras de un corpus. Se utiliza para la incrustación de palabras (la incrustación de palabras es un término utilizado para la representación de palabras para el análisis de textos, normalmente en forma de un vector de valor real que codifica el significado de la palabra, de manera que se espera que las palabras que están más cerca en el espacio vectorial tengan un significado similar (la incrustación de palabras es mucho más rápida que el uso de tf-idf)). GloVe se presentó en Jeffrey Pennington, Richard Socher y Christopher D. Manning. 2014. GloVe: Global Vectors for Word Representation.",
                              "txt_tooltip_fasttext": "FastText es un algoritmo gratuito y de código abierto para obtener representaciones vectoriales de las palabras, que se entrena con webcrawl en inglés y Wikipedia. Se utiliza para la incrustación de palabras (la incrustación de palabras es un término utilizado para la representación de palabras para el análisis de textos, normalmente en forma de un vector de valor real que codifica el significado de la palabra de tal manera que se espera que las palabras que están más cerca en el espacio vectorial sean similares en significado (la incrustación de palabras es mucho más rápida que el uso de tf-idf)",
                              "txt_tooltip_logistic regression": "La regresión logística es un modelo de aprendizaje automático. En primer lugar, aprende de un conjunto de prácticas teniendo en cuenta diferentes factores (por ejemplo, el valor tfidf (véase la información sobre tfidf)), para predecir si una consulta coincide con un documento. A continuación, intentará predecir la probabilidad de que una consulta coincida con un documento. Si ahora introduce una consulta, los documentos se clasificarán según la probabilidad de que coincidan con la consulta introducida.",
                  ...
                  "arabic": {"ID_lang": "arabic",
                             ...
                             "txt_font": "الخَط",
                             "txt_selectStemmer": "اخْتِيَار المُجّذّع",
                             "txt_toggleStopword": "مُستبعدات الفهرسة",
                             "txt_toggleClustering": "التَّجميع (Clustering)",
                             "txt_selectsearchmode": "اختِيار صَنف البَحث",
                             "txt_selectsnowballlang": "snowball لغة المُجذّع",
                             "txt_selectstopwordlang": "لُغة مُستبعدات الفهرسة",
                             "txt_docs_to_return": "عدد المُستنتدات",
                             "txt_on": "نَعَم",
                             "txt_off": "لا",
                             "txt_confirm": "تَم",
                             "txt_cancel": "إلغاء",
                             "txt_tooltip_porter": """مجذّع الكلمات 'بورتر'ّيتّبع الخوارزميّة المقدّمة في             
                              Porter, M 'An algorithm for suffix stripping.' Program 14.3 (1980): 130-137""",
                             "txt_tooltip_lancaster": """مجذّع كلمات مرتكز على خوارزميّة 'لانساستر' يتبّع الخوارزميّة المقدّمة في
                              Paice, Chris D. “Another Stemmer.” ACM SIGIR Forum 24.3 (1990): 56-61.""",
                             "txt_tooltip_snowball": """تطبيق لمجذّع الكلمات 'سنوبول' المُنشأ من قِبَل مارتين بورتر. هذا الخيار مناسب للتجذيع باللّغات: العربيّة, الدنماركيّة, الهولنديّة, الإنجليزيّة, الفنلنديّة, الفرنسيّة, الألمانيّة, الهنغاريّة, الإيطاليّة, النرويجيّة, البرتغاليّة, الرومانيّة, الروسيّة, الإسبانيّة, السويديّة. الخوارزميّة الإنكليزيّة قُدِّمَت في
                             Porter, M. 'An algorithm for suffix stripping.' Program 14.3 (1980): 130-137.""",
                             "txt_tooltip_stemmer": "المُجّذّع هو خوارزميّة تقوم بتخفيف الكلمات المشتقّة إلى جذرها أو جذعها (جزئيّة الكلمة المسؤلة عن المعنى اللغوي لها), على سبيل المثال جذع الكلمات 'كاتب' 'كتاب' 'مكتوب' هو 'كتب'.",
                             ...
                             "txt_tooltip_clustering": "هو عمليّة تجميع أو تصنيف محتويّات البيانات المعطاة تبعاً لسِمة معيّنة (كنوع من التشابه, في هذه الحالة يتم تجميع الكلمات حسب تشابه السّياقات التي تظهر فيها), يتمّ هذا لتسريع عمليّة البحث.",
                             "txt_tooltip_tfidf": "'تردد المصطلح-معكوس تردد الوثيقة' هو عبارة عن إحصاء رقمي يهدف إلى إظهار مدى أهمية كلمة ما لمستند واحد في مجموعة من المستندات. هنا يتم استخدامه كعامل ترجيح للبحث. تزداد قيمة 'تردد المصطلح-معكوس تردد الوثيقة' بشكل متناسب مع عدد المرّات التي تظهر فيها الكلمة في المستند ويتم موازنتها بعدد المستندات المحتوية على الكلمة في المجموعة.",
                             "txt_tooltip_GloVe": """هي خوارزميّة تعلّم غير مراقب أو تعلم استنتاجي للحصول على تمثيلات شعاعيّة للكلمات. يجري التدريب على التواجد المُشترك لثنائي من الكلمات. يتم استخدامها لتضمين الكلمات (تضمين الكلمات هو تمثيل المفردات والمركبات بصورة أشعة رياضية وأعداد حقيقية وهي من طرائق نمذجة اللغات الطبيعية(يعتبر تضمين الكلمات أسرع بكثير من 'تردد المصطلح-معكوس تردد الوثيقة')) تم تقديم هذه الخوارزميّة في
                             Jeffrey Pennington, Richard Socher, and Christopher D. Manning. 2014. GloVe: Global Vectors for Word Representation.""",
                             "txt_tooltip_fasttext": "هي خوارزميّة حرّة ومفتوحة المصدر لتحصيل تمثيلات شعاعيّة للكلمات, يتمّ تدريبها على زاحف شبكة إنجليزي. يتم استخدامها لتضمين الكلمات (تضمين الكلمات هو تمثيل المفردات والمركبات بصورة أشعة رياضية وأعداد حقيقية وهي من طرائق نمذجة اللغات الطبيعية(يعتبر تضمين الكلمات أسرع بكثير من 'تردد المصطلح-معكوس تردد الوثيقة'))",
                             "txt_tooltip_logistic regression": "الانحدار اللوجستي هو نموذج إحصائي ينتمي لنماذج الانحدار الخطي يمكن من نمذجة متغير ثنائي الحد بدلالة مجموعة من المتغيرات العشوائية المتوقعة، رقمية كانت أو فئوية. يستخدم الانحدار اللوجستي للتنبؤ باحتمالية وقوع حدث ما بمعرفة إضافية لقيم متغيرات يمكن أن تكون مفسرة أو مرتبطة بهذا الحدث. يستخدم الانحدارُ اللوجستي عدة متغيرات مُتوقَّعة والتي يمكن أن تكون رقمية أو فئوية.",
